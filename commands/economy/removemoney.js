...
module.exports = {
  ...
  run: async (bot, message, args) => {
    ...
    const user =
      ...
      message.guild.members.cache.find(
        (r) =>
          r.displayName.toLowerCase() === args[0].toLocaleLowerCase()
      );
    ...
    if (!args[1]) {
      return message.channel.send(
        new MessageEmbed()
          .setColor("RANDOM")
          .setDescription("**<a:deny:892076004183506954> | Please Enter An Amount!**")
      );
    }
    ...
    if (args[1] > bal) {
      return message.channel.send(
        new MessageEmbed()
          .setColor("RANDOM")
          .setDescription("**<a:deny:892076004183506954> | Cannot Remove That Much Money!**")
      );
    }
    ...
    message.channel.send(
      new MessageEmbed()
        .setColor("RANDOM")
        .setDescription(
          `<a:check:892071687250673664> | Removed ${args[1]} coins\n\nNew Balance: ${bal2}`
        )
    );
  }
};
