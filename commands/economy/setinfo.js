...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        if (!newInfo) return message.channel.send('**<a:deny:892076004183506954> Please Enter Your Info!**');
        if (newInfo.length > 165) return message.channel.send(`**<a:deny:892076004183506954> Max \`165\` Characters Allowed!**`);
        let newsInfo = chunk(newInfo, 42).join('\n');
        db.set(`info_${user.id}`, newsInfo);

        let notesEmbed = new MessageEmbed()
            .setColor("RANDOM")
            .setAuthor(`<a:check:892071687250673664> Your Info Box Has Been Set`, message.author.displayAvatarURL())
