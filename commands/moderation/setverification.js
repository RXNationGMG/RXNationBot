...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        if (!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send("**<a:deny:892076004183506954> You Do Not Have Required Permissions! - [ADMINISTRATOR]!**");

        if (!args[0]) return message.channel.send("**<a:deny:892076004183506954> Please Enter A Channel Name Where The User Should Be Asked To Verify!**");

        if (!args[1]) return message.channel.send("**<a:deny:892076004183506954> Please Enter A Role Which Will Be Added After The User Is Verified!**");
        ...
        if (!channel || channel.type !== 'text') return message.channel.send("**<a:deny:892076004183506954> Please Enter A Valid Channel!**");
        ...
        try {
            ...
            if (channel.id === a && role.id === b) {
                return message.channel.send('**<a:deny:892076004183506954> This Channel is Already Set As Verification Channel!**');
            } else if (channel.id === a && role.id === b) {
                return message.channel.send("**<a:deny:892076004183506954> This Role is Already Set As Verification Role!**");
            }
            else {
                ...
                return message.channel.send(`**<a:check:892071687250673664> Verification Channel And Role Has Been Set Successfully in \`${channel.name}\`!**`);
            };
        } catch {
