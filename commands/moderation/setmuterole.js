  ...
  run: async (bot, message, args) => {
    ...
    if (!args[0]) {
      ...
      if (message.guild.roles.cache.has(b)) {
        return message.channel.send(
          `**<a:check:892071687250673664> Muterole Set In This Server Is \`${roleName.name}\`!**`
        );
      } else
        return message.channel.send(
          "**<a:deny:892076004183506954> Please Enter A Role Name or ID To Set!**"
        );
    }
    ...
    try {
      ...
      if (role.id === a) {
        return message.channel.send(
          "**<a:deny:892076004183506954> This Role is Already Set As Muterole!**"
        );
      } else {
        db.set(`muterole_${message.guild.id}`, role.id);

        message.channel.send(
          `**<a:check:892071687250673664> \`${role.name}\` Has Been Set Successfully As Muterole!**`
        );
      }
    } catch (e) {
      return message.channel.send(
        "**<a:deny:892076004183506954> Error - `Missing Permissions or Role Doesn't Exist!`**",
        `\n${e.message}`
      );
    }
  }
