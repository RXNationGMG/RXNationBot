...
module.exports = {
    ...
    run: async (bot, message, args, ops) => {
        if (!args[0]) return message.channel.send("**<a:deny:892076004183506954> Please Enter A Song Name!**")
        const url = args[1] ? args[1].replace(/<(.+)>/g, '$1') : '';
        const searchString = args.slice(1).join(' ');

        const { channel } = message.member.voice;
        if (!channel) return message.channel.send("**<a:deny:892076004183506954> You Are Not In A Voice Channel!**");


        const permissions = channel.permissionsFor(message.client.user);
        if (!permissions.has('CONNECT')) {
            return message.channel.send('<a:deny:892076004183506954> I cannot connect to your voice channel, make sure I have the proper permissions!');
        }
        if (!permissions.has('SPEAK')) {
            return message.channel.send('<a:deny:892076004183506954> I cannot speak in this voice channel, make sure I have the proper permissions!');
        }
