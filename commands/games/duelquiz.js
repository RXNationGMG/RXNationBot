...
module.exports = {
  config: {
    ...
    description: 'Answer A Series Of Quiz Questions Against A Human',
  ...
  run: async (bot, message, args, ops) => {
    ...
    if (maxPts <= 0 || maxPts >= 9)
      return message.channel.send('**<a:deny:892076004183506954> | Please Enter A Number Between 1-8!**');

    if (opponent.user.bot)
      return message.channel.send('**<a:deny:892076004183506954> | Cannot Play Against Bots!**');

    if (opponent.user.id === message.author.id)
      return message.channel.send('**<a:deny:892076004183506954> |Cannot Play Against Yourself!**');
    ...
    try {
      ...
      while (!winner) {
        ...
        await message.channel.send({
          embed: {
            ...
            description: `**${question.question}\nPlease Choose A Letter Within ${time / 1000}s**\n\n${question.answers
            ...
            footer: { text: '<a:warn:891035320030724196> | Please Choose The Letter To Answer!' },
          },
        });
        ...
        const filter = (res) => {
          ...
          if (
            question.answers[choices.indexOf(res.content.toUpperCase())] !== question.correct
          ) {
            message.channel.send(`**<a:warn:891035320030724196> | ${res.author}, That's Incorrect!**`).catch(() => null);
            return false;
          }
        ...
        await message.channel.send(`**<a:check:892071687250673664> | Nice One, ${result.author}! The Score Is Now ${score}!**`);
      ...
      if (winner === 'time') return message.channel.send('**<a:warn:891035320030724196> | Game Ended Due To Inactivity!**');
      if (!winner) return message.channel.send('**<a:warn:891035320030724196> | Nobody Won!**');

      return message.channel.send(`**Congrats, ${winner}, You Won!**`);
    } catch (err) {
      ops.games.delete(message.channel.id);
      return message.channel.send('**<a:deny:892076004183506954> | Trivia Not Available!**');
    }
