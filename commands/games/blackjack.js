...
module.exports = {
  ...
  run: async (bot, message, args, ops) => {
    if (!args[0]) return message.channel.send('**<a:deny:892076004183506954> | Please Enter Your Deck Amount!**');
    let deckCount = parseInt(args[0]);
    if (isNaN(args[0])) return message.channel.send('**<a:deny:892076004183506954> | Please Enter A Number!**');
    if (deckCount <= 0 || deckCount >= 9) return message.channel.send("**<a:deny:892076004183506954> | Please Enter A Number Between 1 - 8!**");

    let user = message.author;
    let bal = db.fetch(`money_${user.id}`);
    if (bal === null) bal = 0;
    if (!args[1]) return message.channel.send("**<a:deny:892076004183506954> | Please Enter Your Bet!**");

    let amount = parseInt(args[1]);
    if (isNaN(args[1])) return message.channel.send("**<a:deny:892076004183506954> | Please Enter A Number**");
    if (amount > 10000) return message.channel.send("**<a:deny:892076004183506954> | Cannot Place Bet More Than \`10000\`**");

    if (bal < amount) return message.channel.send("**<a:deny:892076004183506954> | You Are Betting More Than You Have!**");
    ...
    try {
      ...
      if (dealerInitialTotal === 21 && playerInitialTotal === 21) {
        ...
        return message.channel.send('**<a:warn:891035320030724196> | Both Of You Just Hit Blackjack!**');
