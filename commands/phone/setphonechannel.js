    ...
    run: async (bot, message, args) => {
        ...
        if (!args[0]) {
            ...
            if (message.guild.channels.cache.has(b)) {
                return message.channel.send(`*<a:deny:892076004183506954> *Phone Call Channel Set In This Server Is \`${channelName.name}\`!**`);
            } else
        ...
        if (!channel) return message.channel.send("**<a:deny:892076004183506954> Please Enter A Valid Channel Name or ID!**")

        try {
            let a = await db.fetch(`pc_${message.guild.id}`)

            if (channel.id === a) {
                return message.channel.send("**<a:deny:892076004183506954> This Channel is Already Set As Phone Call Channel!**")
            } else {
                bot.guilds.cache.get(message.guild.id).channels.cache.get(channel.id).send("**<a:check:892071687250673664> Phone Call Channel Set!**")
