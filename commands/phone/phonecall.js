...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        if (!phoneid) return message.channel.send("**<a:deny:892076004183506954> No Channels Are Currently Allowing Phone Calls!**")

        let phonelist = phoneid.sort((a, b) => b.ChannelID - a.ChannelID).map(r => r)
            .map(r => r.ChannelID);
        if (message.channel.id !== phonechannel) {
            return message.channel.send('**<a:deny:892076004183506954> You Can Start Call Only In The Set Phone Call Channel!**');
        }
        if (phonechannel === null) return message.channel.send("**<a:deny:892076004183506954> Please Set A Phone Call Channel In Your Server To Call Somebody!**");
        if (inPhoneCall(message.channel)) return message.channel.send('**<a:deny:892076004183506954> This Channel Is Already In A Phone Call!**')
        ...
        if (!channels.size) return message.channel.send('**<a:deny:892076004183506954>No Channels Have Currently Allowed Phone Calls!**');
        let channel;
        if (channelID) {
            channel = channelID
            if (message.guild.channels.cache.has(channel.id)) return message.channel.send("**<a:deny:892076004183506954> Cannot Start Phone Call In The Same Server!**")
            if (!channel || !channel.guild) return message.channel.send('**<a:deny:892076004183506954> This Channel Does Not Exist, Try Again!**');
